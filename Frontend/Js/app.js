...
function getStoredItems() {
    ...
    function initializeItems() {
        ...
        if (stored.length === 0 && itensIniciais.length > 0) {
                ...
                <div class="carousel-inner">
                    ${item.fotos.map((foto, index) => `
                        <div class="carousel-item ${index === 0 ? "active" : ""}">
                            <img src="${foto}" class="d-block w-100" alt="${item.nome || item.descricao}">
                        </div>
                    `).join("")}
                </div>
                ${item.fotos.length > 1 ? `
    ...
    function renderItems(items) {
        ...
        if (items.length === 0) {
            lista.innerHTML = `
                <div class="col-12">
                    <div class="alert alert-warning text-center">Nenhum item encontrado.</div>
                </div>
            `;
            return;
        }
                    ...
                    <div class="card item-card h-100">
                        ...
                        <div class="card-body d-flex flex-column">
                            <div>
                                ...
                                <p class="card-text"><strong>Valor:</strong> ${formatCurrency(item.valor)}</p>
                                <div class="mb-3">
                                    <span class="badge bg-secondary">${item.status}</span>
                                    <span class="badge bg-dark">${item.prazo || "Sem prazo"}</span>
                                </div>
                            </div>
                            <div class="mt-auto">
                                <div class="d-flex justify-content-between mb-3">
                                    <span class="badge bg-success">#${item.id}</span>
                                    <span class="badge bg-secondary">${bidsCount} proposta(s)</span>
                                </div>
                                <button class="btn btn-primary w-100" onclick="window.location.href='recebimento.html?itemId=${item.id}'">
                                    Tenho interesse
                                </button>
                            </div>
                        </div>
    ...
    function filterItems() {
        ...
        const filtered = items.filter(item => {
            const text = [item.nome, item.descricao, item.categoria, item.local].filter(Boolean).join(" ").toLowerCase();
    ...
    window.addEventListener("DOMContentLoaded", () => {
        ...
        document.getElementById("btn-principal")?.addEventListener("click", () => { window.location.href = "principal.html"; });
        document.getElementById("btn-sobre")?.addEventListener("click", () => { window.location.href = "about.html"; });
        document.getElementById("btn-explorar")?.addEventListener("click", () => { window.location.href = "login.html"; });
        ...
        document.querySelectorAll('.navbar [href]').forEach(el => {
            el.addEventListener('click', (e) => {
                ...
                if (href) {
                    // permitir que links <a> funcionem normalmente; para <button href> navegamos manualmente
